...
# usage run.sh <mysql_version> <source_box_name> [source_box_version]
# can pass auto as <mysql_version> to install the current version in apt catalogue
...
if [ "$1" != "auto" ]; then
    export PKR_MYSQL_VER=$1
else
    export PKR_MYSQL_VER=""
fi
